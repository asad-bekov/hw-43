...
Интернет
   ↑
[NAT-инстанс] (192.168.10.254, публичный IP: 51.250.90.182)
   ↑
[Публичная подсеть] (192.168.10.0/24)
 ├── [public-vm] (192.168.10.28, публичный IP: 51.250.76.37)
 └── [Приватная подсеть] (192.168.20.0/24)
      └── [private-vm] (192.168.20.24) ← доступ через NAT
```
...
```
Этот образ не работает с SSH, поэтому не удавалось подключиться к созданным инстансам.

Вместо него был использован рабочий и актуальный образ:
